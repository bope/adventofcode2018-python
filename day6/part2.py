...
def solution1(input, max_dist):
    ...
    size = 0
    for x, ys in dist_map.items():
        for y, _, in dist_map.items():
            if sum(abs(x - cx) + abs(y - cy) for cx, cy in input) < max_dist:
                size += 1
    return size
...
if __name__ == '__main__':
    with open('input.txt') as fh:
        print(solution1(parse_input1(fh.read().strip()), 10000))